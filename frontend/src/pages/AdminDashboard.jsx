  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#FBFBFA] pt-28 text-[#2E2C29]">
      <div className="max-w-7xl mx-auto w-full px-4 md:px-8 mb-24">
        ...
        >
          <StatCard title="Accumulated Revenue" value={`₹${revenue}`} icon={<IndianRupee size={18} />} description="Settled transactions summary" />
          <StatCard title="Unique Guests" value={paidClients} icon={<Users size={18} />} description="Active allocation accounts" />
          <StatCard title="Pending Verifications" value={pendingRequests} icon={<Activity size={18} />} description="Awaiting system authorization" accent={pendingRequests > 0} />
        </motion.div>
            ...
            >
              <div className="bg-[#F7F4EB]/40 border border-[#E6E0D3] rounded-3xl p-6 md:p-10">
                ...
                <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Event Title</label>
                    <input type="text" required className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Category / Sphere</label>
                    <input type="text" required placeholder="e.g. Technology, Music" className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Description Narratives</label>
                    <textarea rows="3" required className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29] resize-none" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Calendar Date</label>
                    <input type="date" required className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Location Coordinates</label>
                    <input type="text" required className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Total Capacity Seats</label>
                    <input type="number" required className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.totalSeats} onChange={(e) => setFormData({ ...formData, totalSeats: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Ticket Valuation Price (INR)</label>
                    <input type="number" required placeholder="0 for Free" className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.ticketPrice} onChange={(e) => setFormData({ ...formData, ticketPrice: e.target.value })} />
                  </div>
                  <div className="md:col-span-2">
                    <label className="block text-[10px] uppercase tracking-wider font-semibold text-[#8C857B] mb-2">Visual asset URL (Optional)</label>
                    <input type="url" placeholder="https://..." className="w-full bg-[#FBFBFA] border border-[#E6E0D3] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#4A5D4E] transition-colors text-[#2E2C29]" value={formData.image} onChange={(e) => setFormData({ ...formData, image: e.target.value })} />
                  </div>
                  <div className="md:col-span-2 pt-2 flex justify-end">
                    <button type="submit" className="bg-[#2E2C29] hover:bg-[#2E2C29]/90 text-[#FBFBFA] text-xs font-medium uppercase tracking-wider py-4 px-8 rounded-xl transition-colors duration-300 shadow-xs cursor-pointer">
                ...
                {events.map((event) => (
                  ...
                  >
                    <div className="truncate pr-4">
                      <p className="font-medium text-sm text-[#2E2C29] truncate group-hover:text-[#4A5D4E] transition-colors">{event.title}</p>
                  ...
                  >
                    <div className="space-y-1.5 grow max-w-full">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <h4 className="text-sm font-medium font-serif text-[#2E2C29] truncate">{b.eventId?.title || "Unknown Experience Asset"}</h4>
                      </div>
                      
                      <p className="text-xs font-light text-[#59554F]">
                        Buyer: <span className="font-normal text-[#2E2C29]">{b.userId?.name}</span> <span className="text-[#8C857B] font-mono text-[11px]">({b.userId?.email})</span>
                      </p>
...
const StatCard = ({ title, value, icon, description, accent = false }) => (
  ...
  >
    <div className="flex justify-between items-start mb-3">
      <span className="text-[10px] uppercase tracking-wider font-semibold text-[#8C857B]">{title}</span>
      <div className={`p-2 rounded-lg ${accent ? "bg-amber-100 text-amber-800" : "bg-[#EBE5D8] text-[#4A5D4E]"}`}>
        {icon}
      </div>
    </div>
    <p className="text-3xl font-serif text-[#2E2C29] tracking-tight mb-1">{value}</p>
